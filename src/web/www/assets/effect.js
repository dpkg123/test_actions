...
decadeModule.import(function(lib, game, ui, get, ai, _status){
	decadeUI.effect = {
				...
				dialog.set = function(attr, value){
					switch (attr) {
						...
						case 'source':
							...
							if (value.isUnseen(0))
								dialog.characters[0].firstChild.setBackground(value.name2,"character");
							else dialog.characters[0].firstChild.setBackground(value.name1,"character");
						...
						case 'target':
							...
							if (value.isUnseen(0))
								dialog.characters[1].firstChild.setBackground(value.name2,"character");
							else dialog.characters[1].firstChild.setBackground(value.name1,"character");
							// var avatar = value.isUnseen(0) ? value.node.avatar2 : value.node.avatar;
		...
		if (lib.config.extension_十周年UI_newDecadeStyle == 'on') {
			...
			if (lib.config['extension_EngEX_HDMode']) scale /= 1.5
			animation.playSpine({
			...
			if (bounds == null) return;
			var sz = bounds.size;
			var scale = Math.min(animation.canvas.width / sz.x, animation.canvas.height / sz.y) * 1.2;
			if (lib.config['extension_EngEX_HDMode']) scale /= 2
			animation.playSpine({
		...
		kill:function(source, target){
			...
			var sourceAvatar = source.isUnseen(0) ? source.name2 : source.name1;
			var targetAvatar = target.isUnseen(0) ? target.name2 : target.name1;
			...
			killerWarpper.killer.setBackground(sourceAvatar,"character");
			...
			victim.back.part1.setBackground(targetAvatar,"character");
			victim.back.part1.style.backgroundSize = "";
			victim.back.part2.setBackground(targetAvatar,"character");
			victim.back.part2.style.backgroundSize = "";
		...
		skill:function(player, skillName, vice){
			...
			image.onload = function () {
				bgImage.onload = function () {
					...
					if (lib.config['extension_EngEX_HDMode']) sprite.scale /= 1.5;
