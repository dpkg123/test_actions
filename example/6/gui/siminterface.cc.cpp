...
//LB to avoid recursive call of bx_atexit()
//     if (!code)
                ...
                case BXT_PARAM_DATA:
                  {
                    ...
                    if (!dparam->is_text_format()) {
                      ...
                      */
                      gzFile zf= gzopen(devdata, "rb");
                      //gzread(zf, dparam->getptr(), dparam->get_size());
                      const unsigned max = 1024*1024*1024;
                      Bit64u len = dparam->get_size();
                      Bit8u *ptr = dparam->getptr();
                      bool loop = true;
                      while(loop) {
                        gzread(zf, ptr, len<=max?len:max);
                        if (len<=max)
                          loop = false;
                        else {
                          len -= max;
    ...
    case BXT_PARAM_DATA:
      {
        ...
        if (!dparam->is_text_format()) {
          ...
          */
          gzFile zf= gzopen(tmpstr, "wb");
          //gzwrite(zf, dparam->getptr(), dparam->get_size());
          const unsigned max = 1024*1024*1024;
          Bit64u len = dparam->get_size();
          Bit8u *ptr = dparam->getptr();
          bool loop = true;
          while(loop) {
            gzwrite(zf, ptr, len<=max?len:max);
            if (len<=max)
              loop = false;
            else {
              len -= max;
