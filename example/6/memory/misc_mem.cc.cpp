...
//LB
// Not in Android to avoid link of c++ lib
#ifndef BX_ANDROID
#include <memory>
#endif
#include "bochs.h"
...
Bit8u* BX_MEM_C::alloc_vector_aligned(Bit64u bytes, Bit64u alignment)
{
  ...
  //LB
#ifndef BX_ANDROID
  BX_MEM_THIS actual_vector = new(std::nothrow) Bit8u [(Bit64u)(bytes + test_mask)];
...
void BX_MEM_C::init_memory(Bit64u guest, Bit64u host)
{
  ...
  //myLog("guest %llu  host %llu", guest, host);
...
#ifdef BX_ANDROID
#if BX_LARGE_RAMFILE
  int hostMBOrig= host/(1024*1024);
  int hostMB= hostMBOrig;
  while(BX_MEM_THIS vector==0 && host>=guest/4) {
	  if (hostMB>=2048)
		  host-= 512*(1024*1024);
	  else if (hostMB>=1024)
		  host-= 256*(1024*1024);
		  ...
		  break;
	  hostMB= host/(1024*1024);
	  BX_MEM_THIS vector = alloc_vector_aligned(host + BIOSROMSZ + EXROMSIZE + 4096, BX_MEM_VECTOR_ALIGN);
  }
  if (hostMB != hostMBOrig) {
	  bx_param_num_c *bxp_host_memsize = SIM->get_param_num(BXPN_HOST_MEM_SIZE);
	  bxp_host_memsize->set(hostMB);
  }
#endif
  if (BX_MEM_THIS vector==0) {
	  BX_FATAL(("alloc_vector_aligned: unable to allocate host RAM !"));
	  return;
  }
#endif
...
//LB
  	...
  	if (guest>host) {
    	...
    	BX_MEM_THIS used_blocks = 0;
      	BX_MEM_THIS overflowFileUsed= true;
  	  	BX_MEM_THIS overflow_file = fopen("memswap.dat", "wb+");
	} else {
#endif
...
void BX_MEM_C::allocate_block(Bit32u block)
{
   ...
   */
  //LB
  if (BX_MEM_THIS overflowFileUsed && BX_MEM_THIS used_blocks >= max_blocks) {
...
void BX_MEM_C::register_state()
{
  ...
  Bit32u num_blocks = (Bit32u)(BX_MEM_THIS len / BX_MEM_BLOCK_LEN);
#if BX_LARGE_RAMFILE
  if (BX_MEM_THIS overflowFileUsed) {
...
Bit8u *BX_MEM_C::getHostMemAddr(BX_CPU_C *cpu, bx_phy_address addr, unsigned rw)
{
  ...
  if (memory_handler) {
    Bit8u *data;
    if (memory_handler->doDirectAccess(a20addr, rw, &data))
      return data;
  }
  // while (memory_handler) {
...
{
  //LB
  if (monitor_addr_t::numMonitorArmed == 0)
    return 0;
