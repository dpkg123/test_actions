...
//LB
int sdlMixerVolume= SDL_MIX_MAXVOLUME;
bx_bool bx_soundlow_waveout_sdl_c::mixer_common(Bit8u *buffer, int len)
{
  Bit32u len2 = 0;

  static int oldLen;
  static Bit8u *tmpbuffer;

  if (len > oldLen) {
	  if (tmpbuffer!=0)
		  delete [] tmpbuffer;
	  tmpbuffer = new Bit8u[len];
	  oldLen= len;
  }
  ...
  for (int i = 0; i < cb_count; i++) {
    if (get_wave[i].cb != NULL) {
      ...
      if (len2 > 0) {
        SDL_MixAudio(buffer, tmpbuffer, len2, sdlMixerVolume);
      }
    }
  }
  return 1;
