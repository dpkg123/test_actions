...
void sb16_init_options(void)
{
  ...
  bx_param_num_c *dmatimer = new bx_param_num_c(menu,
    ...
    0);
  //LB
  bx_param_bool_c *dmadyn = new bx_param_bool_c(menu,
    "dmadyn",
    "Dynamic Timer",
    "Auto recalc DMA timer",
    0);
...
Bit32s sb16_options_parser(const char *context, int num_params, char *params[])
{
  if (!strcmp(params[0], "sb16")) {
    ...
    for (int i = 1; i < num_params; i++) {
      ...
      //LB
      } else if (!strncmp(params[i], "dmadyn=", 7)) {
          bool dmadyn = atol(&params[i][7]);
          SIM->get_param_bool("dmadyn", base)->set(dmadyn);
...
void bx_sb16_c::init(void)
{
  ...
  //LB
  menu->add(SIM->get_param("dmadyn", base));
  ...
  //LB
  SIM->get_param_bool("dmadyn", base)->set_handler(sb16_param_handler);
  ...
  //LB
  lastRealTime= time(NULL);
  lastTicksCount= 0;
  dmadyn = SIM->get_param_bool("dmadyn", base)->get();
}
...
//LB Problem DOS session in Windows95
//       if (DSP.dma.mode != 0)
//       {
...
void bx_sb16_c::dsp_dma(Bit8u command, Bit8u mode, Bit16u length, Bit8u comp)
{
  ...
  //LB
  BX_SB16_THIS sampledatarate= sampledatarate;
...
//LB
bool bx_sb16_c::dsp_RecalcTimer()
{
	if (!BX_SB16_THIS dmadyn)
		return false;
	int timer = BX_SB16_THIS dmatimer;
	Bit64u currRealTime= time(NULL);
	int dt= currRealTime - BX_SB16_THIS lastRealTime;
	if (dt>0) {
		Bit64u currTicks= bx_pc_system.time_ticks();
		Bit64u ticks= currTicks - BX_SB16_THIS lastTicksCount;
		float currIps= ticks/(float)dt;
		static Bit32u ips= SIM->get_param_num(BXPN_IPS)->get();
		float r= currIps/ips + 0.1;
		if (r<0.5) r= 0.5;
		if (r>2) r= 2;
		timer*=r;
		BX_SB16_THIS lastRealTime= currRealTime;
		BX_SB16_THIS lastTicksCount= currTicks;
		DSP.dma.timer = timer * BX_DMA_BUFFER_SIZE / (BX_SB16_THIS sampledatarate);
		return true;
	}
	return false;
}
...
#if BX_NO_LOGGING
void bx_sb16_c::writelog(int, const char*, ...) {}
#else
...
Bit64s bx_sb16_c::sb16_param_handler(bx_param_c *param, int set, Bit64s val)
{
  if (set) {
    ...
    //LB
    } else if (!strcmp(pname, "dmadyn")) {
        BX_SB16_THIS dmadyn = (bool)val;
    } else {
...
void bx_sb16_c::handleResume()
{
	if (sleepTime!=0) {
		Bit64u diff= time(NULL)-sleepTime;
		lastRealTime+= diff;
		sleepTime= 0;
	}
	if (dspe) {
