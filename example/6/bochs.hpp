...
#ifdef BX_ANDROID
extern "C" const char *getAndroidStoragePath();
extern "C" const char *getAndroidFilesPath();
extern "C" const char *getAndroidFilesPathSD();
extern "C" const char *getAndroidAppWorkDir();
extern "C" void JavaExit();
extern "C" void JavaToastMessage(const char *message, int level=0);
#endif
#include <stdio.h>
...
class MyLog {
    const char *funcName;
	public:
	MyLog(const char *funcName) {
	    this->funcName= funcName;
		myLog("%s", funcName);
	}
    ~MyLog() {
		myLog("exiting %s", funcName);
	}
};
#ifndef __QNXNTO__
extern "C" {
...
#if BX_USE_CPU_SMF
#define BX_SET_THE_CPU(x)           { BX_CPU_C::theCpuSaved = BX_CPU_C::theCpu; BX_CPU_C::theCpu = bx_cpu_array[x]; }
#define BX_RESET_THE_CPU            BX_CPU_C::theCpu = BX_CPU_C::theCpuSaved;
#else
#define BX_SET_THE_CPU(x)
#define BX_RESET_THE_CPU
#endif
#else
...
{
//#ifdef ANDROID
#if BX_ANDROID && (__arm__ || __aarch64__)
