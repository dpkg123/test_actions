/////////////////////////////////////////////////////////////////////////
// $Id: main.cc 13879 2020-06-07 17:32:48Z vruppert $
/////////////////////////////////////////////////////////////////////////
//
...
#ifdef BX_ANDROID
#define ANDROID_MSG_SAVED 110
#define ANDROID_MSG_RESTORE_START 111
#define ANDROID_MSG_RESTORE_END 112
#define ANDROID_MSG_STARTED 101
...
extern "C" void myLog(const char *msg, ...) {
  ...
  va_start(valist, msg);
  static int nLines= 0;
  static FILE *f= fopen("mylog.txt", "w");
  fprintf(f, "%d - ", nLines);
...
//LB
#include <dirent.h>
static void removeSavepoint(const char *dirname)
{
	DIR *dir;
	struct dirent *entry;
	char path[PATH_MAX];
    ...
    BX_INFO(("removeDirTree %s", dirname));
	dir = opendir(dirname);
	if (dir == NULL) {
		return;
	}
	...
	while ((entry = readdir(dir)) != NULL) {
	    BX_INFO(("%s", entry->d_name));
		if (strcmp(entry->d_name, ".") && strcmp(entry->d_name, "..")) {
			snprintf(path, (size_t) PATH_MAX, "%s/%s", dirname, entry->d_name);
			//if (entry->d_type == DT_DIR) {
			DIR *tmpdir;
			if ((tmpdir=opendir(path))!=NULL) {
				closedir(tmpdir);
				removeSavepoint(path);
			} else {
				unlink(path);
			}
		}
...
#ifdef BX_ANDROID
//	string configPath= bochsrc_filename;
//	configPath= configPath.substr(0, configPath.find_last_of("/"));
	setenv("BXSHARE", getAndroidFilesPath(), 1);
	setenv("SDCARD", getAndroidStoragePath(), 1);
	setenv("CONFIG", getAndroidAppWorkDir(), 1);
#endif
#if BX_WITH_CARBON
  if(!getenv("BXSHARE"))
  {
  ...
  if (SIM->get_param_bool(BXPN_RESTORE_FLAG)->get()) {
    //LB
    char *path= SIM->get_param_string(BXPN_RESTORE_PATH)->getptr();
...
#ifdef BX_ANDROID
  	// force mouse enabled
	bx_param_bool_c *pb= SIM->get_param_bool(BXPN_MOUSE_ENABLED);
	if (pb!=NULL)
		pb->set(true);
	/*
	// check memory
    bx_param_num_c *pn= SIM->get_param_num(BXPN_MEM_SIZE);
	if (pn!=NULL) {
		long m= pn->get();
    	if (m>2048) {
		  	m= 2048;
...
int bx_begin_simulation(int argc, char *argv[])
{
  ...
  if (SIM->get_param_bool(BXPN_RESTORE_FLAG)->get()) {
#ifdef BX_ANDROID
    Android_JNI_SendMessage(ANDROID_MSG_RESTORE_START, -1);
#endif
...
//LB
  if (SIM->get_param_bool(BXPN_RESTORE_FLAG)->get()) {
    char *path= SIM->get_param_string(BXPN_RESTORE_PATH)->getptr();
    BX_INFO(("Removing checkpoint"));
    removeSavepoint(path);
    BX_INFO(("Checkpoint removed"));
#ifdef BX_ANDROID
    Android_JNI_SendMessage(ANDROID_MSG_RESTORE_END, -1);
#endif
  }
...
#endif
  {
    if (BX_SMP_PROCESSORS == 1) {
      ...
      static Bit32u executed = 0;
      static Bit32u processor = 0;

      if (setjmp(BX_CPU_C::jmp_buf_env))
        BX_CPU_C::jumped = true;
      while (1) {
        // do some instructions in each processor
        BX_SET_THE_CPU(processor)
        BX_CPU(processor)->cpu_run_trace();

        // see how many instruction it was able to run
        Bit32u n = (Bit32u)(BX_CPU(processor)->get_icount() - BX_CPU(processor)->get_icount_last_sync());
        BX_CPU(processor)->sync_icount();
        if (n == 0) n = quantum; // the CPU was halted
        executed += n;

        if (++processor == BX_SMP_PROCESSORS) {
          processor = 0;
          quantum = executed / BX_SMP_PROCESSORS;
          BX_TICKN(quantum);
...
#endif /* BX_DEBUGGER == 0 */
#ifdef BX_ANDROID
    extern bool doSaveState;
    extern char savePath[];
    if (doSaveState) {
    	SIM->save_state(savePath);
        Android_JNI_SendMessage(ANDROID_MSG_SAVED, -1);
    }
#endif
...
int bx_atexit(void)
{
  ...
  if (SIM && SIM->get_init_done()) {
    for (int cpu=0; cpu<BX_SMP_PROCESSORS; cpu++)
      ...
      if (BX_CPU(cpu))
#endif
        { BX_SET_THE_CPU(cpu) BX_CPU(cpu)->atexit(); BX_RESET_THE_CPU }
  }
#endif
#endif
