...
int BX_MAX_TRACE_LENGTH;

void bxICache_c::init() {
  if (initialized)
    return;
  initialized = true;
  cacheTag= MAX_TAG-1;
  //myLog("size cache = %d MB, cache entry = %d bytes", (sizeof(bxICacheEntry_c)*BxICacheEntries)/(1024*1024), sizeof(bxICacheEntry_c));
  //myLog("size pool = %d MB, instruction = %d bytes", (sizeof(bxInstruction_c)*BxICacheMemPool)/(1024*1024), sizeof(bxInstruction_c));
  flushICacheEntries();
  BX_MAX_TRACE_LENGTH =
#if BX_SUPPORT_SMP
    (BX_SMP_PROCESSORS > 1) ? SIM->get_param_num(BXPN_SMP_QUANTUM)->get() :
#endif
    BX_MAX_TRACE_LENGTH_DEF;
}

void flushICaches(void)
{
#if USE_SHARED_ICACHE
  BX_CPU_C::iCache.flushICacheEntries();
...
#if USE_SHARED_ICACHE
  BX_CPU_C::iCache.handleSMC(pAddr, mask);
  for (unsigned i=0; i<BX_SMP_PROCESSORS; i++)
    BX_CPU(i)->async_event |= BX_ASYNC_EVENT_STOP_TRACE;
#else
  for (unsigned i=0; i<BX_SMP_PROCESSORS; i++) {
