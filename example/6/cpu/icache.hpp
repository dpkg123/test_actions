...
#define USE_SHARED_ICACHE 1
extern int BX_MAX_TRACE_LENGTH;
...
#if BX_CPU_LEVEL == 3
#define BxICacheEntries (64 * 1024)  // Must be a power of 2.
#define BxICacheMemPool (128 * 1024)
#elif BX_CPU_LEVEL == 4
#define BxICacheEntries (128 * 1024) // Must be a power of 2.
#define BxICacheMemPool (256 * 1024)
#elif BX_CPU_LEVEL == 5
#define BxICacheEntries (256 * 1024)  // Must be a power of 2.
#define BxICacheMemPool (512 * 1024)
#else
#if BX_SUPPORT_X86_64
#define BxICacheEntries (1024 * 1024)  // Must be a power of 2.
#define BxICacheMemPool (1024 * 1024)
#else
#define BxICacheEntries (512 * 1024)  // Must be a power of 2.
#define BxICacheMemPool (768 * 1024)
#endif
#endif
...
//  return ((pAddr + (pAddr << 2) + (pAddr>>6)) & (BxICacheEntries-1)) ^ fetchModeMask;
  ...
  BX_CPP_INLINE void alloc_trace(bxICacheEntry_c *e)
  {
    ...
    if ((mpindex + BX_MAX_TRACE_LENGTH + 1) > BxICacheMemPool) {
      //myLog("pool exhausted, reset cache");
      flushICacheEntries();
    }
    e->entryTag= cacheTag;
    e->i = &mpool[mpindex];
...
private:
  BX_CPP_INLINE bxICacheEntry_c* _get_entry(bx_phy_address pAddr, unsigned fetchModeMask)
  {
  	bxICacheEntry_c* e= &(entry[hash(pAddr, fetchModeMask)]);
    if (e->entryTag!=cacheTag) {
...
public:
  ...
  {
	  bxICacheEntry_c* e= &(entry[hash(pAddr, fetchModeMask)]);
    if (e->pAddr == pAddr && e->entryTag == cacheTag)
      return e;
    return NULL;
  }
  ...
  {
    if (e->pAddr != BX_ICACHE_INVALID_PHY_ADDRESS && e->entryTag==cacheTag) {
      e->pAddr = BX_ICACHE_INVALID_PHY_ADDRESS;
...
{
  cacheTag= (cacheTag+1)%MAX_TAG;
  if (cacheTag==0) {
	for (int i=0; i<BxICacheEntries; ++i) {
  	entry[i].pAddr= BX_ICACHE_INVALID_PHY_ADDRESS;
  	entry[i].traceMask= 0;
	entry[i].entryTag= 0;
	}
  }
...
BX_CPP_INLINE void bxICache_c::handleSMC(bx_phy_address pAddr, Bit32u mask)
{
  ...
  bxICacheEntry_c *e = _get_entry(LPFOf(pAddr), 0);
